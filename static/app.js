...
// Set initial hidden state — hanya untuk above-fold elements
...
// ── STICKY NAVBAR LOGIC — const STICKY_SCROLL_SHOW = 320;
...
// ── ANALISIS TEKNIKAL: TOGGLE & DETAIL — let _techSectionOpen = false;
...
function renderSummaryCard(d) {
  ...
  // ── METRICS HELPER — const getValue = (metricName, value) => {
    switch (metricName) {
      case 'growth': {
  ...
  // ── VERDICT TEXT — const verdictEl = document.getElementById('summary-verdict');
...
async function doSearch() {
  ...
  try {
    ...
    // --- Merge Local/Regional Data BEFORE rendering — if (resLocal && resLocal.ok) {
      try {
...
/* -- UPGRADE MODAL -- */
  ...
  // ── MODERN RADAR PASAR SENTRA — const SECTOR_CONFIG = {
  ...
  // ── DETAIL ANALYSIS PANEL — let _detailData = null;
